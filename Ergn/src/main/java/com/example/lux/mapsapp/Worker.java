...
public class Worker extends Thread {
	...
	public void run() {
		...
		try {
			...
			//com.example.lux.mapsapp.Worker will receive the matrix and then create C and P according to it
			...
			//Start the training
			while(!stop) {
				//The worker will calculate the new values for R (for the part that he is assigned to)
				System.out.println("Calculating R matrix...");
				calculateX();
				System.out.println("Calculated R matrix");
				//Then he will send back to the server thread all the changes he made on R
				...
				System.out.println("R matrix has been sent");
				//After all the server threads have fully updated the R matrix in the server, the worker will receive it
				System.out.println("Receiving R matrix...");
				for(i=0;i<K;i++) {
					for(j=0;j<rowsUsers;j++) {
						R.setEntry(j,i,in.readDouble());
					}
				}
				System.out.println("R matrix has been received");
				...
				//Then he will send back to the server thread all the changes he made on L
				...
				System.out.println("L matrix has been sent");
				...
				System.out.println("L matrix has been received");
				//Finally the worker will receive a boolean value that indicates whether he must stop the training or not
				System.out.println("Waiting for response from server...");
				//Basically now the server is calculating the score based on the new matrices
				//When thats done, each server thread will send a boolean value back to each worker
				//That value indicates whether or not the workers will continue the training
				stop = in.readBoolean();
				if(!stop) {
					System.out.println("com.example.lux.mapsapp.Server ordered 1 more round of training");
				}
			}
			System.out.println("Training has been completed");
			System.out.println("com.example.lux.mapsapp.Worker is off!");
