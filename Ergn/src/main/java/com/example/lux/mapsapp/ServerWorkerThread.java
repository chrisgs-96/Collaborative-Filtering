...
public class ServerWorkerThread extends Thread {
	...
	public ServerWorkerThread(CountDownLatch latch, int position, WorkerInfo[] workers, int[][] matrix, RealMatrix L, RealMatrix R, double lamda, int alpha) {
		...
		//Pointers to the com.example.lux.mapsapp.Server matrices
	...
	public void run() {
		...
		try {
			...
			System.out.println("com.example.lux.mapsapp.Worker #"+pos+" has received all primary data");
			...
			//Start the training
			while(!stop) {
				//Receive the part of R that the worker (for this thread) has changed
				//Meanwhile copy those changes to the actual R matrix
				for(i=0;i<K;i++) {
				...
				//Notify the server that this thread has updated R matrix
				latch.countDown();
				...
				//Now that the R matrix has been fully updated by each thread, we can send it to the worker
				...
				//The worker will calculate L based on the new R
				//The thread will receive the part of L that the worker (for this thread) has changed
				//Meanwhile the thread will copy the changes to the actual L matrix
				for(i=0;i<K;i++) {
				...
				//Notify the server about the update on L matrix
				latch.countDown();
				...
				//Now that the L matrix has been fully updated we can send it to the worker
				...
				latch.countDown();
				//Wait for response from the server (all threads must have updated and sent L to their workers)
